...
- This suite documents the active route layer in `src/routes/users.py`, `src/routes/user_types_auth.py`, and `src/routes/bulk_operations.py`
...
- Password reset generates a temporary password internally, but the public response only returns expiry/instructions and says the password was delivered out-of-band
