#!/usr/bin/env python3
"""
Database Recreation Script for Magic Auth System

This script DROPS and recreates the database from scratch:
1. Drops the 'magic_auth' database if it exists
2. Creates fresh database
3. Creates all tables in order
4. Creates all stored procedures in order

⚠️  WARNING: This will DELETE ALL DATA in the magic_auth database!
...
def print_warning():
    ...
    print("\n" + "!" * 80)
    print("  ⚠️  WARNING: This will DELETE ALL DATA in the 'magic_auth' database!")
    print("!" * 80)


def confirm_action():
    """Ask user to confirm the destructive action"""
    print("\n  Type 'YES' to confirm database recreation: ", end="")
...
def drop_database(connection):
    """Drop the magic_auth database"""
    print("\n  Dropping existing 'magic_auth' database...", end=" ")
    try:
        cursor = connection.cursor()
        cursor.execute("DROP DATABASE IF EXISTS magic_auth")
        connection.commit()
        cursor.close()
        print("✓ Done")
        return True
...
def recreate_database():
    ...
    print_header("Magic Auth Database Recreation Script")
    print_warning()
    
    # Ask for confirmation
    if not confirm_action():
        print("\n✗ Database recreation cancelled by user.")
        sys.exit(0)
    
    # Get password if not provided
    if not DB_CONFIG['password']:
        DB_CONFIG['password'] = getpass.getpass(f"\nEnter password for {DB_CONFIG['user']}@{DB_CONFIG['host']}: ")
    ...
    try:
        ...
        if not drop_database(connection):
