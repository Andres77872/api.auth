...
-- Safely copies legacy users.email values into pending user_emails rows. These rows do
-- not grant email login and do not become primary until a later token-backed activation.
-- ===================================================================================
DROP PROCEDURE IF EXISTS sp_backfill_legacy_user_emails$$
CREATE PROCEDURE sp_backfill_legacy_user_emails()
BEGIN
    INSERT INTO user_emails (
        id,
        user_id,
    ...
    SELECT CONCAT('ue-legacy-', REPLACE(UUID(), '-', '')),
           u.id,
           LOWER(TRIM(u.email)),
           UNHEX(SHA2(CONCAT('legacy-backfill:v1:', u.id, ':', LOWER(TRIM(u.email))), 256)),
           CASE
               WHEN LOCATE('@', LOWER(TRIM(u.email))) > 1 THEN CONCAT(
                   LEFT(SUBSTRING_INDEX(LOWER(TRIM(u.email)), '@', 1), 1),
                   '***@',
    ...
    WHERE NULLIF(TRIM(u.email), '') IS NOT NULL
      AND NOT EXISTS (
          ...
          WHERE ue.user_id = u.id
            AND ue.email_normalized = LOWER(TRIM(u.email))
            AND ue.removed_at IS NULL
      );

    SELECT ROW_COUNT() AS backfilled_rows,
           'legacy_users.email_pending_backfill' AS backfill_status;
END$$
