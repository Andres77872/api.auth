...
-- Get all child user groups (recursive) for a given parent group
CREATE PROCEDURE sp_get_user_group_descendants(
    IN p_parent_group_id INT
)
BEGIN
    WITH RECURSIVE group_descendants AS (
        ...
        FROM user_groups 
        WHERE id = p_parent_group_id AND is_active = 1
        ...
        SELECT 
            ...
            ug.parent_group_id,
            ug.group_level,
            gd.depth_from_parent + 1
        FROM user_groups ug
        JOIN group_descendants gd ON ug.parent_group_id = gd.id
        WHERE ug.is_active = 1
    )
    SELECT 
        *,
    ...
    LEFT JOIN user_group_members ugm ON gd.id = ugm.user_group_id AND ugm.is_active = 1
    GROUP BY gd.id
    ORDER BY gd.depth_from_parent, gd.group_name;
END$$

-- Get all child permission groups (recursive) for a given parent group within a project
CREATE PROCEDURE sp_get_permission_group_descendants(
    IN p_parent_group_id INT,
    IN p_project_id INT
)
BEGIN
    WITH RECURSIVE perm_group_descendants AS (
        -- Base case: the parent permission group itself
        SELECT 
            id,
            group_hash,
            project_id,
        ...
        -- Recursive case: child permission groups
        SELECT 
            pg.id,
            pg.group_hash,
            pg.project_id,
            ...
            pg.parent_permission_group_id,
            pg.group_level,
            pg.group_priority,
            pgd.depth_from_parent + 1
        FROM permission_groups pg
        JOIN perm_group_descendants pgd ON pg.parent_permission_group_id = pgd.id
        WHERE pg.is_active = 1
    )
    SELECT 
        *,
        COUNT(DISTINCT pgp.permission_id) as permission_count
    FROM perm_group_descendants pgd
    LEFT JOIN permission_group_permissions pgp ON pgd.id = pgp.permission_group_id AND pgp.is_active = 1
    GROUP BY pgd.id
    ORDER BY pgd.depth_from_parent, pgd.group_priority DESC, pgd.group_name;
END$$
