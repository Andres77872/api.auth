    ...
    with get_connection() as con:
        ...
        try:
            ...
            cur.execute("""
                ...
                WHERE permission_id = %s AND is_active = 1
...
def grant_permission_to_group(
    ...
    with get_connection() as con:
        ...
        cur.execute("""
            ...
            WHERE pg.id = %s AND p.id = %s AND pg.is_active = 1 AND p.is_active = 1
...
def revoke_permission_from_group(
    ...
    with get_connection() as con:
        ...
        cur.execute("""
            ...
            WHERE pg.id = %s AND pg.is_active = 1
        """, [permission_group_id])
        ...
        cur.execute("""
            UPDATE permission_group_permissions 
            SET is_active = 0, revoked_at = NOW(), revoked_by = %s
            WHERE permission_group_id = %s AND permission_id = %s AND is_active = 1
...
def get_permission_group_permission(permission_group_id: int, permission_id: int) -> Optional[PermissionGroupPermission]:
    """Get specific permission group permission link"""
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
            SELECT id, permission_group_id, permission_id, granted_at, granted_by, revoked_at, revoked_by, is_active
...
def get_permission_group_permissions(permission_group_id: int) -> List[Permission]:
    ...
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
            SELECT p.id, p.permission_hash, p.project_id, p.permission_name, p.permission_display_name,
                   p.permission_description, p.permission_category, p.is_system_permission,
                   p.created_at, p.updated_at, p.created_by, p.is_active
            FROM permissions p
            JOIN permission_group_permissions pgp ON p.id = pgp.permission_id
            WHERE pgp.permission_group_id = %s AND p.is_active = 1 AND pgp.is_active = 1
...
def get_user_permission_group_assignment(user_id: int, project_id: int, permission_group_id: int) -> Optional[UserProjectPermissionGroup]:
    """Get specific user permission group assignment"""
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
            SELECT id, user_id, project_id, permission_group_id, assigned_at, assigned_by, removed_at, removed_by, is_active
...
def get_user_effective_permissions(user_id: int, project_id: int) -> List[Permission]:
    ...
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
            SELECT DISTINCT p.id, p.permission_hash, p.project_id, p.permission_name, p.permission_display_name,
                   p.permission_description, p.permission_category, p.is_system_permission,
                   p.created_at, p.updated_at, p.created_by, p.is_active
            FROM permissions p
            JOIN permission_group_permissions pgp ON p.id = pgp.permission_id
...
def get_project_audit_log(project_id: int, limit: int = 100, offset: int = 0) -> List[PermissionAuditLog]:
    ...
    with get_connection() as con:
        cur = con.cursor()
        cur.execute("""
            SELECT id, action_type, project_id, target_user_id, permission_id, permission_group_id,
                   performed_by, old_values, new_values, action_timestamp, ip_address, user_agent
...
def get_user_permission_summary(user_id: int, project_id: int) -> Optional[UserPermissionSummary]:
    ...
    with get_connection() as con:
        ...
        cur.execute("""
            ...
            WHERE u.id = %s AND p.id = %s AND u.is_active = 1 AND p.is_active = 1
        """, [user_id, project_id])
